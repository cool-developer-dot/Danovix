import { SOCIAL_JOURNEY } from "@/lib/contact/constants";
...
type Network = (typeof SOCIAL_JOURNEY.networks)[number]["network"];

function SocialGlyph({ network }: { network: Network }) {
  const className = "relative z-[1] h-5 w-5";
...
export function SocialJourney() {
  return (
    <section
      aria-labelledby="social-journey-heading"
      className={socialRoot}
    >
      <div data-contact="social" className={socialInner}>
        <p className={contactEyebrowDark}>{SOCIAL_JOURNEY.eyebrow}</p>
        <h2 id="social-journey-heading" className={socialHeading}>
          {SOCIAL_JOURNEY.heading}
        </h2>
        <p className={socialDescription}>{SOCIAL_JOURNEY.description}</p>

        <div className={socialRow} role="list">
          {SOCIAL_JOURNEY.networks.map((network) => (
            <a
              key={network.id}
              href={network.href}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`Follow DANOVIX on ${network.label}`}
              role="listitem"
              className={socialButton}
            >
              <span className={socialGlow} aria-hidden="true" />
              <SocialGlyph network={network.network} />
            </a>
          ))}
        </div>

        <a
          href={SOCIAL_JOURNEY.ctaHref}
          target="_blank"
          rel="noopener noreferrer"
          className={socialCta}
        >
          {SOCIAL_JOURNEY.cta}
          <span className={socialCtaArrow} aria-hidden="true">
            →
          </span>
        </a>
      </div>
